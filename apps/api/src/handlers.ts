...
import { runsTable, screenshotsTable, urlsTable } from './db/schema'
...
export const postUrls = async (c: Context<{ Bindings: Env }>) => {
  const db = drizzle(c.env.D1)
...
export const getScreenshots = async (c: Context<{ Bindings: Env }>) => {
  const db = drizzle(c.env.D1);
...
export const uploadToR2 = async (c: Context<{ Bindings: Env }>) => {
...
export const getLatest = async (c: Context<{ Bindings: Env }>) => {
  const db = drizzle(c.env.D1)
...
export const getLanguages = async (c: Context<{ Bindings: Env }>) => {
  const db = drizzle(c.env.D1)
...
export const getAvailableDates = async (c: Context<{ Bindings: Env }>) => {
  const db = drizzle(c.env.D1)
  const urlParam = c.req.query('url')
...
export const getStatus = async (c: Context<{ Bindings: Env }>) => {
  const db = drizzle(c.env.D1)
  const runs = await db
    .select()
    .from(runsTable)
...
export const postRuns = async (c: Context<{ Bindings: Env }>) => {
  const db = drizzle(c.env.D1)
  const body = await c.req.json<{
...
export const getImage = async (c: Context<{ Bindings: Env }>) => {
