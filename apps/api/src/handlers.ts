...
export const deleteUrl = async (c: Context<{ Bindings: Env }>) => {
	...
	const result = await db.delete(urlsTable).where(eq(urlsTable.id, id)).run();
...
export const postScreenshots = async (c: Context<{ Bindings: Env }>) => {
	...
	if (
		...
		return c.json(
			{
				error: "url_id, device, job_status, and created_at are required",
			},
			400,
		);
	}
	...
	async (c) => {
		...
		const latestSubquery = db
			.select({
				url_id: screenshotsTable.url_id,
				device: screenshotsTable.device,
				maxCreatedAt: sql<string>`MAX(${screenshotsTable.created_at})`.as(
					"max_created_at",
				),
			})
			.from(screenshotsTable)
			.where(eq(screenshotsTable.device, deviceType))
			.groupBy(screenshotsTable.url_id, screenshotsTable.device)
			.as("sq");
